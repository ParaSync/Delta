...
export const updateNodeById = (nodes: Node[], id: string, props: Record<string, any>): Node[] => {
...
// Create new node
export const createNode = (type: NodeType, defaultProps: Record<string, any> = {}): Node => {
  const id = generateId();
  const node: Node = {
    id,
    type,
  ...
  if (['text', 'textarea', 'number', 'date', 'time', 'datetime', 'select', 'multiselect', 'radio', 'checkbox', 'file', 'table'].includes(type)) {
    node.key = generateFieldKey(defaultProps.label || type);
  }
...
// Validate form data
export const validateFormData = (data: Record<string, any>, nodes: Node[]): Record<string, string> => {
